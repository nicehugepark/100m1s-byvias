# ByBias — 정적 사이트 (GitHub Pages 배포용)
...
- `CNAME` — 커스텀 도메인 `bybias.100m1s.com`
...
3. (커스텀 도메인) DNS에 **CNAME 레코드**: `bybias` → `<계정>.github.io`
   - Settings → Pages → Custom domain에 `bybias.100m1s.com` 입력, "Enforce HTTPS" 체크
